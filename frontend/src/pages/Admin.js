...
const Admin = () => {
  const [train, setTrain] = useState({ trainNumber: '', source: '', destination: '', totalSeats: '' });
  ...
  const handleAddTrain = async () => {
    ...
    const response = await fetch('http://localhost:3000/admin/addTrain', {
      method: 'POST',
      headers: {
  ...
  return (
    <div>
      ...
      <input type="text" name="trainNumber" placeholder="Train Number" onChange={handleChange} required />
